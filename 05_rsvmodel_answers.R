...
parms <- c(alpha = 5,         # alpha = daily contacts
           beta = 0.3,        # beta = probability of infection on contact from symptomatic person
           gamma = 0.1,       # gamma = probability of infection on contact from asymptomatic person
...
seair_ode <- function(times,init,parms){
  with(as.list(c(parms,init)), {
    # ODEs
    dS <- mu*(S+E+A+I+R) + omega*R - alpha*beta*(I/(S+E+A+I+R))*S - alpha*gamma*(A/(S+E+A+I+R))*S - mu*S 
    dE <- alpha*beta*(I/(S+E+A+I+R))*S + alpha*gamma*(A/(S+E+A+I+R))*S - (1-epsilon)*phi*E - epsilon*phi*E - mu*E
