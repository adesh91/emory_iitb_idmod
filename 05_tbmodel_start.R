...
times <- seq(0, 100, by = 1) # for 10000 years, by year
...
ggplot(out_long, aes(x = time, y = log(value), color = variable, group = variable)) +
  geom_line(size = 1.2) +
  xlab("Time (years)") + ylab("log(Population)") +
  theme_minimal() +
...
out_df %>%
  mutate(N = S + E1 + E2 + I + R ,
         Ip100k = ,# WRITE EXPRESSION FOR INCIDENCE PER 100k
         Mp100k = ,# WRITE EXPRESSION FOR MORTALITY PER 100k 
         inf_prev = (E1 + E2 + I + R) / N) %>%   # WRITE EXPRESSION FOR INFECTION PREVALENCE PER 100k
  tail(5)
...
# Question 4. How sensitive is incidence to changes in pi (the effective contact rate)
